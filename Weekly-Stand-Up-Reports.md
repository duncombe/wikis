...
#### Issues  
* Initial tests of other catalogs (to find wind data) did not yield any results; will investigate further (e.g. for other geographies) before documenting in a progress report
* ASA team has multiple ideas for new notebooks, need to discuss next steps and ensure we're not duplicating effort with other.  Examples include (1) determine whether sea ice is a factor when modeling oil spills (too much overlap with polar bears & ice?); (2) Water temperature - need a scenario simpler than cold pool to start, e.g. Can we discover and access high enough resolution data to support recreational activities (surfing, swim races) or given a fish death event, can we find evidence of hypoxia? More generally, can we compare obs, model, satellite data, etc.; (3) Can we use USGS rapid deploy gages to discover info about overland inundation? 